...
const getCashFlowData = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: allTransactions.slice(0, 100), // Limit for performance
...
const getCashFlowSummary = async (req, res) => {
  try {
    ...
    const totalIncome = salesData.reduce((sum, sale) => sum + (sale.total_revenue || 0), 0);
    const totalExpenses = totalIncome * 0.7; // Mock expenses at 70% of income
    ...
    const summary = {
      ...
      operating_cash_flow: netCashFlow * 0.9,
      free_cash_flow: netCashFlow * 0.8,
      cash_flow_margin: totalIncome > 0 ? (netCashFlow / totalIncome) * 100 : 0
    };

    // Generate income breakdown by marketplace
    const incomeBreakdown = [];
    const marketplaceGroups = {};
    
    salesData.forEach(sale => {
      const marketplace = sale.marketplace || 'Direct Sales';
      ...
      marketplaceGroups[marketplace].amount += sale.total_revenue || 0;
      marketplaceGroups[marketplace].transactions_count++;
    });

    Object.values(marketplaceGroups).forEach(group => {
      group.percentage = totalIncome > 0 ? (group.amount / totalIncome) * 100 : 0;
      group.growth_rate = Math.random() * 20 - 10; // Mock growth rate
      incomeBreakdown.push(group);
    });
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      const predictedIncome = totalIncome * growthFactor;
      const predictedExpenses = totalExpenses * growthFactor * 0.98; // Slightly slower expense growth
      const predictedNetFlow = predictedIncome - predictedExpenses;
