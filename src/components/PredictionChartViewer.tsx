...
export function PredictionChartViewer({ 
  chartData, 
  forecastData, 
  processedData, 
  forecastMetrics,
  ...
  const renderChart = (height: number = 400) => {
    ...
    if (zoomedData.length > 0) {
      console.log('🔍 Chart Data Sample:', {
        ...
        hasRevenue: zoomedData.some(d => d.revenue !== undefined),
        hasPredicted: zoomedData.some(d => d.predicted !== undefined),
        hasDemand: zoomedData.some(d => d.demand_quantity !== undefined),
    ...
    const tooltipFormatter = (value: any, name: string) => {
      ...
      // Detect if this is stock forecasting (demand_quantity) or sales forecasting (revenue)
      const isStockData = name.includes('demand') || name.includes('stock');
      const isSalesData = name.includes('revenue') || name.includes('predicted') || name.includes('orders');
      ...
      if (isStockData) {
        ...
        // For other numeric values, also round to whole numbers
        const roundedValue = Math.round(Number(value));
        formattedValue = `${roundedValue.toLocaleString('id-ID')}`;
      }
            ...
            <LineChart {...commonProps}>
              ...
              <YAxis 
                ...
                tickFormatter={(value) => {
                  ...
                  if (zoomedData.some(d => d.demand_quantity !== undefined)) {
                    // Round demand values to whole numbers for Y-axis labels
              ...
              {/* Historical Data Line - Auto-detect type */}
              <Line
                type="monotone"
                dataKey={zoomedData.some(d => d.demand_quantity !== undefined) ? "demand_quantity" : "revenue"}
                stroke="#2563eb"
                strokeWidth={2}
                dot={false}
                name={zoomedData.some(d => d.demand_quantity !== undefined) ? "Historical Demand" : "Historical Revenue"}
                animationDuration={animate ? 1500 : 0}
                connectNulls={false}
              />
              
              {/* Forecast Data Line - Auto-detect type */}
              <Line
                type="monotone"
                dataKey={zoomedData.some(d => d.predicted_demand !== undefined) ? "predicted_demand" : "predicted"}
                stroke="#dc2626"
                strokeWidth={3}
                strokeDasharray="5 5"
                dot={false}
                name={zoomedData.some(d => d.predicted_demand !== undefined) ? "Forecast Demand" : "Forecast Revenue"}
                animationDuration={animate ? 1500 : 0}
                connectNulls={false}
              />
        ...
        return (
          <ResponsiveContainer width="100%" height={height}>
            <AreaChart {...commonProps}>
              ...
              {/* Historical Data Area - Auto-detect type */}
              <Area
                type="monotone"
                dataKey={zoomedData.some(d => d.demand_quantity !== undefined) ? "demand_quantity" : "revenue"}
                stroke="#2563eb"
              ...
              {/* Forecast Data Area - Auto-detect type */}
              <Area
                type="monotone"
                dataKey={zoomedData.some(d => d.predicted_demand !== undefined) ? "predicted_demand" : "predicted"}
                stroke="#dc2626"
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              <BarChart3 className="w-5 h-5" />
              {zoomedData.some(d => d.demand_quantity !== undefined) ? 'Stock Demand Forecasting Chart' : 'Sales Revenue Forecasting Chart'}
            </CardTitle>
          ...
          <p className="text-sm text-gray-600">
            {zoomedData.some(d => d.demand_quantity !== undefined) 
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-4 pt-4 border-t">
                ...
                <div className="text-center">
                  ...
                  <div className="font-semibold">{zoomLevel[0]}% - {zoomLevel[1]}%</div>
                </div>
              </div>
            </div>
          ) : (
